...
class MockTracerBN(threading.Thread):
    ...
    def run(self):
        ...
        # Request for load voltage.
            ...
            # Load voltage returned is x100. 2 bytes. 10.00V to 20.00V.
        ...
        # Request for PV current.
            ...
            # PV current returned is x100. 2 bytes. 0.00A to 50.00A.
            self.send(get_two_byte_bytearray(randrange(0, 5000)))

        # Request for PV voltage.
        elif rx_request == RequestCodes.pv_voltage:
            # PV voltage returned is x100. 2 bytes. 0.00V to 150.00V.
            self.send(get_two_byte_bytearray(randrange(0, 15000)))

        # Request for PV power.
        elif rx_request == RequestCodes.pv_power:
            # PV power returned is x100. 4 bytes. 0.00W to 1500.00W.
            self.send(get_four_byte_bytearray(randrange(0, 1500)))

        # Request for PV max voltage today.
        elif rx_request == RequestCodes.pv_voltage_max_today:
            # Max voltage today returned is x100. 2 bytes. 0.00V to 150.00V.
            self.send(get_two_byte_bytearray(randrange(0, 15000)))

        # Request for PV min voltage today.
        elif rx_request == RequestCodes.pv_voltage_min_today:
            # Min voltage today returned is x100. 2 bytes. 0.00V to 150.00V.
            self.send(get_two_byte_bytearray(randrange(0, 15000)))
